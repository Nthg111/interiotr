...
type Props = React.ImgHTMLAttributes<HTMLImageElement> & {
  /** CSS object-position value to control focal point. Defaults to 'center 100%' */
  objectPosition?: string;
  /** When true, render the image as a CSS background to guarantee cropping behavior */
  coverAsBackground?: boolean;
};

export default function Image({ src, alt, className = "", objectPosition = "center 100%", style, coverAsBackground = false, ...rest }: Props) {
  ...
  const combinedStyle = { ...(style || {}), objectPosition, ...(inferredObjectFit ? { objectFit: inferredObjectFit } : {}) } as React.CSSProperties;
  ...
  if (coverAsBackground) {
    return (
      <div
        ...
        style={{ backgroundImage: `url(${src})`, backgroundPosition: objectPosition, backgroundSize: 'cover', backgroundRepeat: 'no-repeat' }}
        role="img"
        aria-label={alt}
      >
